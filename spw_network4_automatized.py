...
# -*- coding: utf8 -*-
'''
Created by Szabolcs Káli
Modified by András Ecker on 08.07.2015
'''
...
def replay():
    '''
    decides if there is a replay or not, and pushbacks the results to matrix: X
    :param : 
...
title = ax1.set_title('Average replay interval')
...
title = ax2.set_title('Mean firing rates')
savefig('replay_and_firing_rates.png')
...
ax1.legend()
ax1.set_xlim(first, last)
title = ax1.set_title('Maximum autocerrelations')
ax2 = fig.add_subplot(2, 1, 2)
...
ax2.set_xlabel('scale factors')
ax2.set_xlim(first, last)
title = ax2.set_title('Maximum autocerrelations in ripple range')
savefig('autocorrelations.png')

# saving result array (X)    
fname = 'results.txt'
np.savetxt(fname, X, fmt='%.6f', delimiter='\t', header='Multiplier, Mean_exc.rate, Max.exc.AC., at[ms], Max.exc.AC.in_ripple_range, at[ms],  Mean_inh.rate, Max.inh.AC., at[ms], Max.inh.AC.in_ripple_range, at[ms], replay, avg. replay interval')
