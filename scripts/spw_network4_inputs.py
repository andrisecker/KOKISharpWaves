...
pop = 'Bas'  # ['Pyr', 'Bas'] population which gets the outer input
inputType = 'Recorded'  # ['Poisson', 'InhPoisson', 'Recorded']
fOut = 'input_Pyr.txt'
...
if inputType == 'Poisson':
    # outer input (Poisson group) MF = PoissonGroup(#{pop}, rate*Hz)
...
elif inputType == 'InhPoisson':
    # outer input (InhomegenPoisson group) MF = PoissonGroup(#{pop}, rates=lambda t: (offset+np.cos(scale*t))*rate*Hz)
    offset = 2
    scale = 13
...
elif inputType == 'Recorded':
    # recorded spike train from other simulations (SpikeMonitor.spikes format...)  MF = SpikeGeneratorGroup(#{pop}, spiketrian)
    ...
    # compress the spikes of 4000 neuron to 1000 neuron
    spikesCompressed = []
    for i in spikes:
        if i[0] < 1000:
            z = (i[0], i[1])
            spikesCompressed.append(z)
        else:
            for j in range(1, 4):
                if i[0] < (1000 + j * 1000):
                    z = (int(i[0] - j * 1000)), i[1]
                    spikesCompressed.append(z)

assert pop in ['Pyr', 'Bas']
assert inputType in ['Poisson', 'InhPoisson', 'Recorded']
...
if inputType in ['Poisson', 'InhPoisson']:
    ...
    for k in range(0, dataPoints):
        ...
        if pop == 'Pyr':
            ...
            MF = PoissonGroup(NI, rates=lambda t: (2+np.cos(13*t))*rate_mf*Hz)
    ...
    if pop == 'Pyr':
        MF = SpikeGeneratorGroup(NE, spikes)
        Cext = IdentityConnection(MF, PE, 'g_ampa', weight=J_PopMF)
        Cei = Connection(PE, PI, 'g_ampa', weight=J_BasExc, sparseness=eps_pyr, delay=delay_BasExc)
        Cie = Connection(PI, PE, 'g_gaba', weight=J_PyrInh, sparseness=eps_bas, delay=delay_PyrInh)
        ...
        MF = SpikeGeneratorGroup(NI, spikesCompressed)
        Cext = IdentityConnection(MF, PI, 'g_ampa', weight=J_PopMF)
    ...
    if pop == 'Pyr':
        ...
        figName = os.path.join(SWBasePath, 'figures', 'recordedST_pyr.png')
        fig2.savefig(figName)
    ...
    figName = os.path.join(SWBasePath, 'figures', 'recordedST__bas.png')
    fig3.savefig(figName)
    ...
    if pop == 'Pyr':
        ...
        figName = os.path.join(SWBasePath, 'figures', 'recordedST_pyr_rate.png')
        fig4.savefig(figName)
    ...
    figName = os.path.join(SWBasePath, 'figures', 'recordedST_bas_rate.png')
    fig5.savefig(figName)
